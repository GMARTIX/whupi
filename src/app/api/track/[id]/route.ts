...
export async function GET(
  ...
  try {
    ...
    // 2. Si el pedido está en camino, buscar ubicación del rider en Pediclub
    if (order.status === 'ACCEPTED' || order.status === 'PICKED_UP' || order.status === 'DELIVERING' || order.status === 'DELIVERED') {
      try {
